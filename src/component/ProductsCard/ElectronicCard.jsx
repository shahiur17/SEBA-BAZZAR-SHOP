const ElectronicCard = ({ data }) => {
  ...
  const getElectronicItems = (array) => {
    if (Array.isArray(array)) {
      return array.filter((item) => item.name.includes("Electronic"));
    }
    return [];
  };

  const topElectronicItems = getElectronicItems(data).slice(0, 20); // Get the top 10 fashion items
